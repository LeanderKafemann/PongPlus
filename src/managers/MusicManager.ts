...
export class MusicManager {
  ...
  private playNote(): void {
    ...
    this.gainNode.gain.exponentialRampToValueAtTime(0.01, this.audioContext.currentTime + 0.25);

    this.oscillator.connect(this.gainNode);
    this.gainNode.connect(this.audioContext.destination);
    this.oscillator.start();
    this.oscillator.stop(this.audioContext.currentTime + 0.25);

    // Next note
    this.currentNote = (this.currentNote + 1) % this.melody.length;

    // Faster tempo: 250ms instead of 500ms
    setTimeout(() => this.playNote(), 250);
  }
